...
<?php
if (!empty($_GET['search']) && $_GET['search2']) {
    ...
    where bustable.TimetableID=timetable.TimetableID and bustable.BusID=bus.BusID and bustable.SeatID=seat.SeatID and timetable.Date='$search' and timetable.Time='$search2';";
    ...
    bus.BusID, bus.Numseat,
    seat.SeatID, seat.SeatCost from bustable, timetable, bus, seat 
    where bustable.TimetableID=timetable.TimetableID and bustable.BusID=bus.BusID and bustable.SeatID=seat.SeatID and timetable.Date='$search';";
    ...
    bus.BusID, bus.Numseat,
    seat.SeatID, seat.SeatCost from bustable, timetable, bus, seat 
    where bustable.TimetableID=timetable.TimetableID and bustable.BusID=bus.BusID and bustable.SeatID=seat.SeatID and timetable.Time='$search2';";
} else {
    $sqlShowBustable = "select bustable.TimetableID as tID, bustable.BusID as bID, bustable.SeatID as sID,
    timetable.TimetableID, timetable.Date, timetable.Time,
    bus.BusID, bus.Numseat,
    seat.SeatID, seat.SeatCost from bustable, timetable, bus, seat 
    where bustable.TimetableID=timetable.TimetableID and bustable.BusID=bus.BusID and bustable.SeatID=seat.SeatID;";
}
?>
<!doctype html>
<html lang="en">
...
<head>
    ...
    <title>Main_CheckSeat2</title>
    <script>
        function confselect(rec) {
            var ans;
            ans = confirm('ท่านต้องการดูที่นั่ง = ' + rec);
            if (ans == true) {
                window.open('Main_CheckSeat3.php?TimetableID=' + rec, '_self');
            } else
                alert('ไม่ต้องการดูรายการนี้');
        }
    </script>
    <style>
        .style-sidebar {
        ...
        <div class="sidebar-sticky style-sidebar">
          ...
          <ul class="nav flex-column mb-2">
            ...
            <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
                ...
                <h5 align="center">รายละเอียดตารางเที่ยวรถ</h5><br>
                <div class="table-style">
                    ...
                    <thead>
                        ...
                        $checkround = 1;
                        $checkroundall = 1;
                        $lashrow = mysqli_num_rows($resultShowBustable);

                        while ($recnumShowBustable = mysqli_fetch_array($resultShowBustable)) {
                            if ($checkround == 1) {
                                $chkDate = $recnumShowBustable['Date'];
                                $chkTime = $recnumShowBustable['Time'];
                                $chkBusID = $recnumShowBustable['BusID'];
                                $chkNumseat = $recnumShowBustable['Numseat'];
                                $chkSeatCost = $recnumShowBustable['SeatCost'];
                                $chkTimetableID = $recnumShowBustable['TimetableID'];
                            }
                            if ($recnumShowBustable['TimetableID'] == $chkTimetableID) {
                                ...
                                echo "<tbody>"; ?><tr class="bg-light text-center"><?php echo "
                                    ...
                                    <td>$chkNumseat</td>
                                    <td>$chkSeatCost</td>"; ?>
                                <td align='center'><button onclick="confselect('<?php echo $chkTimetableID; ?>')">แสดงที่นั่ง</button></td>
                            <?php "</tr><tbody>";
                                $chkDate = $recnumShowBustable['Date'];
                                $chkTime = $recnumShowBustable['Time'];
                                $chkBusID = $recnumShowBustable['BusID'];
