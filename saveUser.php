...
if(isset($_REQUEST['uploadimage'])){ 
    ...
    $sqlimage = "insert into photo(image) value('$image')";
    mysqli_query($connect,$sqlimage);
...
while(true){
	...
	if($res == null){
       ...
       window.open('AddUser2.php?FName=$FName&LName=$LName&Gender=$Gender&Tel=$Tel&Email=$Email&LocationName=$LocationName&Date=$Date&Time=$Time&numSeat=$numSeat&SeatID=$SeatID','_self');</script>";
    }
    else{
        ...
        while($row=mysqli_fetch_array($res2)){
            ...
            value('','$dateTime','$image','$TotalPrice','$LocationName','2','$TimetableID','$nSeat[0]','$row[CustomerID]','')";
        ...
        while($row2=mysqli_fetch_array($resselect)){
            ...
            for($i=1;$i<=$num;$i++){
                ...
                value('$BookingID','$dateTime','$image','$TotalPrice','$LocationName','2','$TimetableID','$nSeat[$i]','$row[CustomerID]','')";
    ...
    echo "<script>alert('เก็บข้อมูลเรียบร้อยแล้ว');window.open('SelectSeat.php?TimetableID=$TimetableID','_self');</script>";
    }
