...
<?php
  if(!empty($_GET['search'])&&$_GET['search2']){
    ...
    $sqlShowTimetable = "select * from Timetable where Date LIKE '%" . $search ."%' and Time LIKE '%" . $search2 ."%'";
  }
  elseif(!empty($_GET['search'])){
    $search = $_REQUEST['search'];
    $sqlShowTimetable = "select * from Timetable where Date LIKE '%" . $search ."%'";
  }
  elseif(!empty($_GET['search2'])){
    $search2 = $_REQUEST['search2'];
    $sqlShowTimetable = "select * from Timetable where Time LIKE '%" . $search2 ."%'";
  }
  else{
    $sqlShowTimetable = "select * from Timetable";
  }
?>
<!doctype html>
<html lang="en">
    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4"><div class="chartjs-size-monitor " style="position: absolute; left: 0px; top: 0px; right: 0px; bottom: 0px; overflow: hidden; pointer-events: none; visibility: hidden; z-index: -1;"><div class="chartjs-size-monitor-expand" style="position:absolute;left:0;top:0;right:0;bottom:0;overflow:hidden;pointer-events:none;visibility:hidden;z-index:-1;"><div style="position:absolute;width:1000000px;height:1000000px;left:0;top:0"></div></div><div class="chartjs-size-monitor-shrink" style="position:absolute;left:0;top:0;right:0;bottom:0;overflow:hidden;pointer-events:none;visibility:hidden;z-index:-1;"><div style="position:absolute;width:200%;height:200%;left:0; top:0"></div></div></div>
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            ...
            <div class="btn-toolbar mb-2 mb-md-0">
                <div class="btn-group mr-2"><a class="btn btn-outline-success" href="Main_TimetableAdd.php" role="button">เพิ่ม</a></div>
            </div>
        </div>
        <?php
        ...
        ?>
          <form  class="form-inline" action= 'Main_Timetable.php' method='get'>
        ...
        <?php
            ...
            if($resultShowTimetable==null)
            {
                echo "คำสั่งผิด";
            }
            
            while($recnumShowTimetable = mysqli_fetch_array($resultShowTimetable))
            {
              echo "<tbody>"; ?><tr class="bg-light text-center small"><?php echo "
                        <td>$recnumShowTimetable[Date]</td>
                        <td>$recnumShowTimetable[Time]</td>
                        <td align='center'>$recnumShowTimetable[Username]</td>
                        <td align='center'><a href='Main_Bustable.php?TimetableID=$recnumShowTimetable[TimetableID]'>แสดงตารางเที่ยวรถ</a></td>"; ?>
                        <td align='center'><button onclick="confedit('<?php echo $recnumShowTimetable['TimetableID']; ?>')">จัดการเที่ยวรถทัวร์</button></td>
                        <td align='center'><button onclick="confdel('<?php echo $recnumShowTimetable['TimetableID']; ?>')">ลบ</button></td> 
                <?php "</tr></tbody>";  
            }
        ?>
        </table>
