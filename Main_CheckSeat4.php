...
<head>
    ...
    <title>Main_CheckSeat4</title>
    <script>
        function confvar(rec, rec2) {
            var ans;
            ans = confirm('ท่านต้องการตรวจสอบรายการนี้ = ' + rec);
            if (ans == true) {
                window.open('Main_CheckPayment.php?BookingID=' + rec + '&TimetableID=' + rec2, '_self');
            } else
                alert('ไม่ตรวจสอบรายการนี้');
        }

        function confsub(rec) {
            var ans;
            ans = confirm('ท่านต้องการยืนยันรายการนี้ = ' + rec);
            if (ans == true) {
                //alert('ลบรายการนี้');
                window.open('#.php?BookingID=' + rec, '_self');
            } else
        ...
        function confdel(rec, rec2) {
            ...
            if (ans == true) {
                window.open('delCancelSeat.php?BookingID=' + rec + '&TimetableID=' + rec2, '_self');
            } else
            ...
            <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
                ...
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    ...
                    <div class="btn-toolbar mb-2 mb-md-0">
                        <div class="btn-group mr-2"><a class="btn btn-outline-danger" href="Main_CheckSeat3.php?TimetableID=<?php echo $TimetableID ?>" role="button">ย้อนกลับ</a></div>
                    </div>
                </div>
                <?php
                require("mysql/config.php");
                require("mysql/connect.php");
                $sqlshowTimetable = "select * from timetable where TimetableID='$TimetableID';";
                $resultShowTimetable = mysqli_query($connect, $sqlshowTimetable);
                while ($recShowTimetable = mysqli_fetch_array($resultShowTimetable)) {
                ?><h5 align="center">ตรวจสอบรายการที่นั่ง</h5>
                    <h6 align="center">(Date : <?php echo $recShowTimetable['Date']; ?>&nbsp;&nbsp;
                        Time : <?php echo $recShowTimetable['Time']; ?>)</h6><?php
                                                                            }
                                                                                ?>
                <div class="table-style">
                    <table class="table table-striped shadow-lg p-3 mb-5 bg-white rounded">
                        ...
                        $sqlShowBooking = "select booking.BookingID, booking.Bookingdate, booking.Payment, booking.TotalPrice, booking.Destination, 
                               ...
                               from booking, timetable where booking.TimetableID=timetable.TimetableID and booking.TimetableID='$TimetableID' and (booking.BookingStatus=2 or booking.BookingStatus=3)";
                        ...
                        while ($recnumShowBooking = mysqli_fetch_array($resultShowBooking)) {
                            ...
                            if ($recnumShowBooking['BookingID'] == $chkBookingID) {
                                ...
                                echo "<tbody>"; ?><tr class="bg-light text-center small"><?php echo "
                                    ...
                                    <td>$chkBookingdate</td>";
                                                                                            ...
                                                                                            ?><td align='center'><button onclick="confvar('<?php echo $chkBookingID; ?>','<?php echo $chkTimetableID; ?>')">ตรวจสอบการชำระเงิน</button></td>
                                    <?php }
                                                                                            echo "<td>$chkTotalPrice</td>
                                    <td>$BookingStatus</td>
                                    <td align='center'><a href='Main_Customer.php?CustomerID=$chkCustomerID&TimetableID=$TimetableID'>ดูข้อมูลลูกค้า</a></td>"; ?>
                                    <!--<td align='center'><button onclick="confsub('<?php echo $chkBookingID; ?>')">แก้ไข</button></td>-->
                                    <td align='center'><button onclick="confdel('<?php echo $chkBookingID; ?>','<?php echo $chkTimetableID; ?>')">ยกเลิก</button></td>
                                <?php "</tr><tbody>";
                                $TotalSeat = $recnumShowBooking['SeatID'] . " ";
                                $chkBookingID = $recnumShowBooking['BookingID'];
                                $chkDate = $recnumShowBooking['Date'];
                            ...
                            if ($checkroundall == $lashrow) {
                                if ($recnumShowBooking['BookingStatus'] == '2') {
                                    $BookingStatus = 'รอการยืนยัน';
                                } elseif ($recnumShowBooking['BookingStatus'] == '3') {
                                    $BookingStatus = 'ยืนยัน';
                                }
                                echo "<tbody>"; ?>
                                <tr class="bg-light text-center small"><?php echo "
                                    ...
                                    <td>$recnumShowBooking[Bookingdate]</td>";
                                                                        ...
                                                                        ?><td align='center'><button onclick="confvar('<?php echo $recnumShowBooking['BookingID']; ?>','<?php echo $recnumShowBooking['TimetableID']; ?>')">ตรวจสอบการชำระเงิน</button></td>
                                    <?php
                                                                        } //<td><img src='images_user/".$recnumShowBooking['Payment']."'></td>
                                                                        echo "
                                    <td>$recnumShowBooking[TotalPrice]</td>
                                    <td>$BookingStatus</td>
                                    <td align='center'><a href='Main_Customer.php?CustomerID=$recnumShowBooking[CustomerID]&TimetableID=$TimetableID'>ดูข้อมูลลูกค้า</a></td>"; ?>
                                    <!--<td align='center'><button onclick="confsub('<?php echo $recnumShowBooking['BookingID']; ?>')">แก้ไข</button></td>-->
                                    <td align='center'><button onclick="confdel('<?php echo $recnumShowBooking['BookingID']; ?>','<?php echo $recnumShowBooking['TimetableID']; ?>')">ยกเลิก</button></td>
                            <?php "</tr><tbody>";
                            }
                            $checkroundall++;
                        }
