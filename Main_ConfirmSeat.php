...
<head>
  ...
  <title>Main_ConfirmSeat</title>
  <script>
    function confvar(rec, rec2) {
      var ans;
      ans = confirm('ท่านต้องการตรวจสอบรายการนี้ = ' + rec);
      if (ans == true) {
        window.open('Main_ConfirmPayment.php?BookingID=' + rec + '&Payment=' + rec2, '_self');
      } else
    ...
    function confdel(rec, rec2, rec3) {
      ...
      if (ans == true) {
        window.open('delConfirmSeat.php?BookingID=' + rec + '&TimetableID=' + rec2 + '&CustomerID=' + rec3, '_self');
      } else
  ...
  <div class="container-fluid">
    <div class="row">
      ...
      <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
        ...
        <h5 align="center">ตรวจสอบการยืนยันการซื้อที่นั่ง</h5><br>
          ...
          <table class="table table-striped shadow-lg p-3 mb-5 bg-white rounded">
            ...
            while ($recnumShowBooking = mysqli_fetch_array($resultShowBooking)) {
              ...
              if ($recnumShowBooking['BookingID'] == $chkBookingID) {
               ...
               <td>$chkTotalPrice</td>
               <td>$BookingStatus</td>
               <td align='center'><a href='Main_CustomerCon.php?CustomerID=$chkCustomerID'>ดูข้อมูลลูกค้า</a></td>"; ?>
                  <td align='center'><button onclick="confsub('<?php echo $chkBookingID; ?>','<?php echo $chkTimetableID; ?>')">ยืนยัน</button></td>
                  <td align='center'><button onclick="confdel('<?php echo $chkBookingID; ?>','<?php echo $chkTimetableID; ?>','<?php echo $chkCustomerID; ?>')">ปฏิเสธ</button></td>
                <?php "</tr><tbody>";
                $TotalSeat = $recnumShowBooking['SeatID'] . " ";
                $chkBookingID = $recnumShowBooking['BookingID'];
                $chkDate = $recnumShowBooking['Date'];
              ...
              if ($checkroundall == $lashrow) {
               ...
               <td>$BookingStatus</td>
               <td align='center'><a href='Main_CustomerCon.php?CustomerID=$recnumShowBooking[CustomerID]'>ดูข้อมูลลูกค้า</a></td>"; ?>
                  ...
                  <td align='center'><button onclick="confdel('<?php echo $recnumShowBooking['BookingID']; ?>','<?php echo $recnumShowBooking['TimetableID']; ?>','<?php echo $recnumShowBooking['CustomerID']; ?>')">ปฏิเสธ</button></td>
              <?php "</tr><tbody>";;
              }
              $checkroundall++;
            }
            /*
              ...
              echo "$TotalSeat";
                ...
                echo "<tr>
                        ...
                        <td>$recnumShowBooking[Destination]</td>
                        <td>$recnumShowBooking[Bookingdate]</td>
                        <td>$recnumShowBooking[Payment]</td>
                        <td>$recnumShowBooking[TotalPrice]</td>
                        <td>$recnumShowBooking[BookingStatus]</td>
                        <td align='center'><a href='Main_Bustable.php?TimetableID=$recnumShowBooking[CustomerID]'>ดูข้อมูลลูกค้า</a></td>
                        <td align='center'><a href='Main_Bustable.php?TimetableID=$recnumShowBooking[TimetableID]'>ยืนยัน</a></td>"; ?>
                        <td align='center'><button onclick="confedit('<?php echo $recnumShowBooking['TimetableID']; ?>')">แก้ไข</button></td>
                        <td align='center'><button onclick="confdel('<?php echo $recnumShowBooking['TimetableID']; ?>')">ลบ</button></td> 
                <?php "</tr>";  */
