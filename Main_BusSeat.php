...
<html lang="en">
  <head>
    ...
    <title>Main_BusSeat</title>
      <script>
        function confedit(rec,rec1)
          {
            var ans;
            ans = confirm('ท่านต้องการแก้ไขรายการนี้ = ' + rec + '&' + rec1 );
            if(ans == true){
              window.open('editBusSeat.php?BusID=' + rec + '&SeatID=' + rec1,'_self');
            }
            else
          ...
          function confdel(rec)
          {
            ...
            if(ans == true){
                //alert('ลบรายการนี้');
              window.open('delSeat.php?BusID=' + rec ,'_self');
            }
            else
    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4"><div class="chartjs-size-monitor " style="position: absolute; left: 0px; top: 0px; right: 0px; bottom: 0px; overflow: hidden; pointer-events: none; visibility: hidden; z-index: -1;"><div class="chartjs-size-monitor-expand" style="position:absolute;left:0;top:0;right:0;bottom:0;overflow:hidden;pointer-events:none;visibility:hidden;z-index:-1;"><div style="position:absolute;width:1000000px;height:1000000px;left:0;top:0"></div></div><div class="chartjs-size-monitor-shrink" style="position:absolute;left:0;top:0;right:0;bottom:0;overflow:hidden;pointer-events:none;visibility:hidden;z-index:-1;"><div style="position:absolute;width:200%;height:200%;left:0; top:0"></div></div></div>
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            ...
            <div class="btn-toolbar mb-2 mb-md-0">
                <div class="btn-group mr-2"><a class="btn btn-outline-danger" href="Main_Bus.php" role="button">ย้อนกลับ</a></div>
                <!-- <div class="btn-group mr-2"><a class="btn btn-outline-success" href="Main_SeatAdd.php" role="button">เพิ่ม</a></div> -->
            </div>
        </div>
        <?php $BusID = $_REQUEST['BusID']; ?>
        <h5 align = "center">รายละเอียดสถานะที่นั่ง<?php echo "($BusID)"; ?></h5><br>
        
        <div class="table-style">
          <table class="table table-striped shadow-lg p-3 mb-5 bg-white rounded">
            <thead>
              <tr class="bg-info text-center small">
            <th>เลขป้ายทะเบียนรถทัวร์</th>
            <th>ที่นั่ง</th>
			      <th>สถานะ</th>
            <th>แก้ไข</th>
            <!--<th>ลบ</th>-->
        ...
        <?php
            ...
            $sqlShowBusSeat = "select * from BusSeat where BusID = '$BusID';";
            $resultShowBusSeat = mysqli_query($connect,$sqlShowBusSeat);

          

            if($resultShowBusSeat==null){
                  echo "คำสั่งผิด";
            }

            else{
        
              while($recnumShowBusSeat = mysqli_fetch_array($resultShowBusSeat))
              {
                if($recnumShowBusSeat['SeatStatus']=='1'){
                  $str1 = "ที่นั่งปกติ";
                }
              if($recnumShowBusSeat['SeatStatus']=='0'){
                  $str1 = "ที่นั่งเสีย";
                }
    
                echo "<tbody>"; ?><tr class="bg-light text-center small"><?php echo "
                          <td>$recnumShowBusSeat[BusID]</td>
                          <td>$recnumShowBusSeat[SeatID]</td>
                          <td>$str1</td>"; ?>
                          <td align='center'><button onclick="confedit('<?php echo $recnumShowBusSeat['BusID']; ?>','<?php echo $recnumShowBusSeat['SeatID']; ?>')">แก้ไข</button></td>
                          <!--  
                          <td align='center'><button onclick="confdel('<?php echo $recnumShowBusSeat['BusID']; ?>','<?php echo $recnumShowBusSeat['SeatID']; ?>')">ลบ</button></td> 
                          -->
                  <?php "</tr></tbody>";   
              }
