...
<html lang="en">
  ...
  <body>
    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4"><div class="chartjs-size-monitor " style="position: absolute; left: 0px; top: 0px; right: 0px; bottom: 0px; overflow: hidden; pointer-events: none; visibility: hidden; z-index: -1;"><div class="chartjs-size-monitor-expand" style="position:absolute;left:0;top:0;right:0;bottom:0;overflow:hidden;pointer-events:none;visibility:hidden;z-index:-1;"><div style="position:absolute;width:1000000px;height:1000000px;left:0;top:0"></div></div><div class="chartjs-size-monitor-shrink" style="position:absolute;left:0;top:0;right:0;bottom:0;overflow:hidden;pointer-events:none;visibility:hidden;z-index:-1;"><div style="position:absolute;width:200%;height:200%;left:0; top:0"></div></div></div>
        ...
        <h5 align="center">เพิ่มเที่ยวรถ</h5><br>

        <?php
            $TimetableID = $_REQUEST['TimetableID'];
            require("mysql/config.php");
            require("mysql/connect.php");
            $sql = "select * from Timetable where TimetableID='$TimetableID'";
            $result = mysqli_query($connect,$sql);
            $row = mysqli_fetch_array($result);

        ?>

        <form name='BusAdd' action="saveAddBustable.php" method="post">
            <div class="form-group col-md-3">
                <label>วันที่เดินทาง :</label>
                <input type="hidden" class="form-control" name="TimetableID" value = "<?=$row['TimetableID']?>">
                <input type="date" class="form-control" name="Date" readonly value = "<?=$row['Date']?>">
            </div>
            <div class="form-group col-md-3">
                <label>เที่ยวรถ :</label>
                <input type="text" class="form-control" name="Time" readonly value = "<?=$row['Time']?>">
            </div>
            <div class="form-group col-md-3">
                <label>รถทัวร์ :</label>
                    <select class="form-control" name="BusID">
                      <option value='' selected>กรุณาเลือกรถทัวร์</option>
                        <?php
                          require("mysql/config.php");
                          require("mysql/connect.php");
                      
                          $sqlSearchBus = "select * from Bus;";
                          $resultSearchBus = mysqli_query($connect,$sqlSearchBus);

                          if($resultSearchBus == null){
                            echo "คำสั่งผิด";
                          }
                          while($recnumSearchBus = mysqli_fetch_array($resultSearchBus)){
                            echo "<option value='$recnumSearchBus[0]'>$recnumSearchBus[0]</option>";
                          }
